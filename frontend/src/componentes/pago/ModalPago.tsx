...
export const ModalPago: React.FC<Props> = ({ abierto, alCerrar, alFinalizar }) => {
  ...
  const manejarEnvioPago = async () => {
    ...
    try {
      ...
      // Reemplaza por la URL de tu backend en Render si ya está desplegado:
      const urlBackend = import.meta.env.VITE_API_URL || 'https://tu-backend-en-render.onrender.com/api';
      ...
      const resPago = await axios.post(`${urlBackend}/transacciones`, {
        ...
        cliente: {
          nombreCompleto: datosEntrega.nombreCompleto || 'Cliente Prueba',
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm">
      <div className="bg-white w-full max-w-md rounded-2xl shadow-xl overflow-hidden flex flex-col max-h-[90vh]">
        ...
        <div className="px-6 py-4 bg-slate-50 border-b flex justify-between items-center">
          ...
          <button onClick={alCerrar} className="text-slate-400 hover:text-slate-600">&times;</button>
        </div>
            ...
            <div className="space-y-3">
              <div>
                ...
                <div className="relative">
                  ...
                  />
                  <span className="absolute right-3 top-2 text-xs font-bold text-slate-500">{franquicia}</span>
                </div>
              ...
              <div className="grid grid-cols-2 gap-2">
                <div>
                  <label className="block text-xs font-medium text-slate-700 mb-1">Expiración (MM/AA)</label>
                  <div className="flex space-x-1">
                    <input
                      ...
                      className="w-1/2 px-2 py-2 border rounded-lg text-sm text-center"
                    />
                    <input
                      type="text"
                      maxLength={2}
        ...
        <div className="px-6 py-4 bg-slate-50 border-t flex justify-between items-center">
          {pasoActual === 2 ? (
            <button
              ...
              disabled={!tarjetaValida || !datosEntrega.direccion}
            ...
            <div className="flex w-full space-x-2">
              <button
                ...
                className="w-1/3 py-2.5 bg-slate-200 text-slate-700 rounded-xl text-sm font-medium"
              >
                Atrás
              </button>
              <button
                ...
                disabled={cargando}
                className="w-2/3 py-2.5 bg-green-600 hover:bg-green-700 text-white rounded-xl text-sm font-medium transition-colors"
              >
